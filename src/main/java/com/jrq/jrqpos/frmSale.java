...
//                }
    ...
    private void addNewRow(DefaultTableModel model, String productId, String productName, String uom, double SRP) {
        model.addRow(new Object[]{productId, productName, uom, getInputQTY(), String.format("%.2f", SRP), String.format("%.2f", SRP)});
    }

    public void AddSale(String getRef, String ModePayment) {
        ...
        try {
            DBCon.Open();
            db.AddSale(SaleID, getUserID, getTotal, getCashAmount, getChange, Date(), Time(), lblTItem.getText(), getRef, ModePayment);
            DBCon.Close();
                ...
                if (GetQty == 0) {
                    ...
                    for (int i = 0; i < model.getRowCount(); i++) {
                        if (model.getValueAt(i, 0).toString().equals(GetID)) {
                            ...
                            double newQty = CurrentQty + getInputQTY();
